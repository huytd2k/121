...
Tetris2::Tetris2(int socket, std::string name, std::string nameLogin) {
...
bool Tetris2::sendVector2D(int socket, const std::vector<std::vector<std::uint32_t>>& data) {
    std::size_t rows = 20;
    std::size_t cols = 10;
...
void Tetris2::draw() {
    ...
    if (gameover) {
        ...
        if (!sendResult) {
            ...
            sendWrapper(gameSocket, message.c_str(), message.size(), 0);
            char buffer[1024];
